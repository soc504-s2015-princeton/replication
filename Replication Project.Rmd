...
#get total producton units, seprating out oaxaca
prod_units[is.na(prod_units)] <- 0
prod.units.main1 <- prod_units %>%
  filter(state != "OAXACA") %>%
  select(name, total_prod_units = V3) %>%
  mutate(total_prod_units = as.numeric((total_prod_units)))

prod.units.oax <- prod_units %>%
  filter(state == "OAXACA") %>%
  select(name, total_prod_units = V3) %>%
  mutate(total_prod_units = as.numeric((total_prod_units)))

#calculate percentages for land surface area according to property type
land[is.na(land)] <- 0

#separating out oaxaca
land.main <- filter(land, state != "OAXACA")
land.oax <- filter(land, state == "OAXACA")

land.main1 <- land.main %>%
  ...
  mutate(total_area = as.numeric((total_area)), 
         ...
         prop.ej = ejidal/total_area *100, 
         prop.com = comunal/total_area * 100, 
...
#separating out oaxaca
subs.main <- filter(subs, state != "OAXACA")
subs.oax <- filter(subs, state == "OAXACA")

subs.main1 <- subs.main %>%
...
#separating out oaxaca
cattle.main <- filter(cattle, state != "OAXACA")
cattle.oax <- filter(cattle, state == "OAXACA")

cattle.main1 <- cattle.main %>%
