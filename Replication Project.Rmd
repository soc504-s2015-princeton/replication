...
homs.for.map <- select(homicide.rate, hom.rate, muncode)
...
land1 <- land %>%
  ...
  mutate(total_area = as.numeric((total_area)), 
         ...
         prop.ej.comm = (ejidal+comunal)/total_area *100, #NEEDS TO BE SEPARATED
...
#NEEDS TO BE LOG YIELDS IN TONS PER HECTARE
...
![Table 2: Coefficients from the Negative Binomial Regression Models of Homicide on Collective Ownership, Agricultural Organization, and Production of Cash Crops (Hypotheses 1 and 2)](data/Table2a.png)
...
glm.nb(hom.rate ~ prop.ej.comm + corn_tons + pop.dens + prct.young + prop.indi + prop.no.lit + pct.fem.house + docs.per.10k + dummy.SOM, data = sample) #we are missing two independent variables in this model: % agricultural units 5 hectares or less, and Thiel's land inequality

glm.nb(hom.rate ~ corn_tons + pop.dens + prct.young + prop.indi + prop.no.lit + pct.fem.house + docs.per.10k + dummy.SOM, data = sample)
