...
main.df$log.corn.yield <- gsub("NA", "0", main.df$log.corn.yield)
main.df$log.corn.yield <- gsub("-Inf", "0", main.df$log.corn.yield)
...
# addsqkm = sum(sqkm), log.pop.dens = log10(total.pop/sqkm), log persons per HA?
```
...
sample.main <- sample.main %>%
  select(muncode, dummy.SOM, prop.less.2500, pct.indi, no.lit.rate, docs_10k, pct.fem.house, pct.young, hom.rate, pct.ej, pct.com, pct.individual, log.corn.yield, pct.cattle, log.pop.surface,  pct.coffee, gini, pol1k, HDI, weigh.elev)
