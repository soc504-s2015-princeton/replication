...
main.df <- main.df %>%
  ...
  select(-indi, -illiteracy, -pop.less.2500, -fem.house, -total.house, -young.males, -hom_total, -ejidal, -surface, -comunal, -private, -total.units, -corn.tons, -corn.ha, -total.cattle.units, -total.coffee.units, -mun.name, -state.name)
#add log.pop.dens = log10(total.pop/sqkm), log persons per HA?


main.df$log.corn.yield[main.df$log.corn.yield == "-Inf"] <- 0
main.df$log.corn.yield[is.na(main.df$log.corn.yield)] <- 0
#main.df$log.pop.surface <- gsub("-Inf", "0", main.df$log.pop.surface)
