...
main.df <- main.df %>%
  ...
  select(-illiteracy, -indi, -pop.less.2500, -fem.house, -total.house, -young.males, -hom_total, -ejidal, -surface, -comunal, -private, -total.units, -corn.tons, -corn.ha, -total.cattle.units, - total.coffee.units, -mun.name, -state.name)

#add log.pop.dens = log10(total.pop/sqkm)
...
sample.main <- main.df %>%
  ...
  select(-name, -sd.elev, -total.pop, -pct.poor, -pct.extpoor)
```
...
oaxaca.d <- oaxaca.d  %>% 
  ...
  summarise(pop.less.2500 = sum(pop.less.2500), total.pop = sum(total.pop), prop.less.2500 = pop.less.2500/total.pop,
            ...
            pol1k = sum(pol1k)) %>%
  select(-illiteracy, -indi, -pop.less.2500, -fem.house, -total.house, -young.males, -hom_total, -ejidal, -surface, -comunal, -private, -total.units, -corn.tons, -total.cattle.units, -corn.tons, -corn.ha) %>%
  mutate(muncode = distrito + 20000)

# addsqkm = sum(sqkm), log.pop.dens = log10(total.pop/sqkm), 
```
...
sample.main <- sample.main %>%
  select(muncode, dummy.SOM, prop.less.2500, pct.indi, no.lit.rate, docs_10k, pct.fem.house, pct.young, hom.rate, pct.ej, pct.com, pct.individual, log.corn.yield, pct.cattle, log.pop.surface,  pct.coffee, gini, pol1k, HDI)

sample.oax <- sample.oax %>%
  select(muncode, dummy.SOM, prop.less.2500, pct.indi, no.lit.rate, docs_10k, pct.fem.house, pct.young, hom.rate, pct.ej, pct.com, pct.individual, log.corn.yield, pct.cattle, sd.elev, log.pop.surface,  pct.coffee, gini, pol1k, HDI)
