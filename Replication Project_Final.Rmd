...
```{r, warning=FALSE, error=FALSE}
#load main 1990 censo database from inegi to the town level, latin1 option is used
## in order for R to recognize special characters
...
#9998 and 9999 are not towns fields - they are information fields.
less.2500 <- censo.1 %>%
...
df.main <- df.main %>%
  ...
  select(-twn, -no.literacy, -indi, -pop.less.2500, -doctors, -fem.house, -total.house, -young.total, -sqkm, -ejidal, -total_area, -comunal, -private, -up19, -up24, -up29, -total, -total_prod_units, -total_units, -corn.tons, -corn.ha, -total.cattle.units, -subs_units, - total.coffee.units)
...
oaxaca.dist <- oaxaca.dist  %>% 
  ...
  summarise(pop.less.2500 = sum(pop.less.2500), 
            ...
            log.avg.plot = log(area.total/plot.total)) %>%
  select(-no.literacy, -indi, -pop.less.2500, -doctors, -fem.house, -total.house, -young.total, -sqkm, -ejidal, -total_area, -comunal, -private, -total_prod_units, -corn.tons, -total.cattle.units, -subs_units, -total_units, -corn.tons, -corn.ha) %>%
...
sample.main <- sample.main %>%
  select(muncode, total.pop, dummy.SOM, prop.less.2500, pct.indi, no.lit.rate, docs.per.10k, pct.fem.house, pct.young, log.pop.dens, hom.rate, pct.ej, pct.com, pct.individual, pct.subs, log.corn.yield, pct.cattle, sd.elev, log.pop.surface, pct.upto5, pct.coffee, theil, log.avg.plot, weigh.elev, hom.total)
...
gpclibPermit() #needed by MapTools
```
...
## bind the data for maps from the main and oaxaca data frames
#map.data <- rbind(main.for.map, oax.for.map)

## read the database file from the map files provided by INEGI. 
## we could not dissolve the 570 Oaxaca municipalities into 30 distrticts in R, 
## we dissolved the Oaxaca polygons using QGIS
# map.dbf <-read.dbf("data/maps/municipios1995-distritos.dbf")

## create the muncode vector in the map databaes
# #map.dbf <- map.dbf %>%
#    #mutate(CVE_MUN = as.numeric(as.character(CVE_MUN)),
#    #       CVE_ENT = as.numeric(as.character(CVE_ENT)),
#    #       muncode = (CVE_ENT*1000) + CVE_MUN)

## join the data for the maps with the map database
#map.merged <- left_join(map.dbf, map.data, by = "muncode" )

##keep ordering from original map database so that polygons can be identified correctly
# map.merged <- map.merged[order(map.merged$OID), ]

## save the original map database without the data from our data sample
# #write.dbf(map.dbf, "data/maps/old_municipios1995-distritos.dbf")

## write the modified map database so that the shapefile can call it.
#write.dbf(map.merged, "data/maps/municipios1995-distritos.dbf")


## we are taking the intervales form those used by Villarreal
...
# 
# ## create slots for data from our sample
# p <- ggplot(map.shp@data, aes(sd_elev, hom_rate))
# 
# ## modify shapefile form and identify muncode as region
# map_geom <- fortify(map.shp, region = "muncode")
# 
# ## merge shapfile with saved slots for information from our data sample
# map_geom <- merge(map_geom, map.shp@data, by.x="id",  by.y="muncode")
# 
# ## create breaks for intervals for homicides
# map_geom$hom_breaks <- cut(map_geom$hom.rate, breaks = breaks_hom, labels = labels_hom, include.lowest = TRUE)
# 
# ## create breaks for intervales for sd elevation
...
![Homicide Rate 2006 - 2008](map_hom1992.png)


```{r, warning=FALSE, error=FALSE}
# ## create map for homicide rate
...
# ggsave("map_hom1992.pdf")
```

**Map of Standard Deviation of Elevation**

![Std. Deviation of Elevation](map_elev1992.png)
```{r, warning=FALSE, error=FALSE}
...
One.a <- glm.nb(hom.total ~ pct.upto5 + theil + log.corn.yield + log.pop.dens + pct.young + pct.indi + no.lit.rate + pct.fem.house + docs.per.10k + dummy.SOM + offset(log(total.pop*3)), data = sample) 

One.b <- glm.nb(hom.total ~ log.avg.plot + theil + log.corn.yield + log.pop.dens + pct.young + pct.indi + no.lit.rate + pct.fem.house + docs.per.10k + dummy.SOM + offset(log(total.pop*3)), data = sample) 

One.c <- glm.nb(hom.total ~ log.pop.surface + theil + log.corn.yield + log.pop.dens + pct.young + pct.indi + no.lit.rate + pct.fem.house + docs.per.10k + dummy.SOM + offset(log(total.pop*3)),data = sample) 
```
...
Two.a <- glm.nb(hom.total ~ pct.ej + pct.com + pct.upto5 + theil + log.corn.yield + log.pop.dens + pct.young + pct.indi + no.lit.rate + pct.fem.house + docs.per.10k + dummy.SOM + offset(log(total.pop*3)), data = sample)
 
Two.b <- glm.nb(hom.total ~ pct.ej + pct.com + pct.individual + pct.coffee + pct.cattle + pct.upto5 + theil + log.corn.yield + log.pop.dens + pct.young + pct.indi + no.lit.rate + pct.fem.house + docs.per.10k + dummy.SOM + offset(log(total.pop*3)), data = sample) 
 
Two.c <- glm.nb(hom.total ~ pct.ej + pct.com + pct.subs + pct.coffee + pct.cattle + pct.upto5 + theil + log.corn.yield + log.pop.dens + pct.young + pct.indi + no.lit.rate + pct.fem.house + docs.per.10k + dummy.SOM + offset(log(total.pop*3)), data = sample) 
```
...
```{r, warning=FALSE, error=FALSE}
Three <- glm.nb(hom.total ~ weigh.elev + pct.ej + pct.com + pct.individual + pct.coffee + pct.cattle + pct.upto5 + theil + log.corn.yield + log.pop.dens + pct.young + pct.indi + no.lit.rate + pct.fem.house + docs.per.10k + dummy.SOM + offset(log(total.pop*3)), data = sample) 
```
