...
stargazer(One.a, One.b, One.c, type = "text", title = "Table 1: Coefficients from the Negative Binomial Regression Models of Homicide on Land Scarcity and Inequality", style = "ajs", covariate.labels=c("Plot size and crop yields: Log persons per hectare", ". Log maize yields", "Demographic variables: Log population density", ". % young males", ". % indigenous", "Resource Deprivation: Illiteracy rate", "Family Structure: % female-headed households", "Access to medical attention: Medical doctors per 10,000", "Geographical Region: Valley of Mexico"), dep.var.labels.include = FALSE, column.labels = c("Model 1", "Model 2", "Model 3"), model.numbers = FALSE, keep.stat = c("n", "ll", "chi2"), notes = "SEs in parentheses")

screenreg(list(One.a, One.b, One.c), caption = "Table 1: Coefficients from the Negative Binomial Regression Models of Homicide on Land Scarcity and Inequality", caption.above = TRUE, stars = c(.05, .01), custom.coef.names = c("Constant", "Log maize yields", "Log population density", ". % young males", ". % indigenous", "Illiteracy rate", "% female-headed households", "Medical doctors per 10,000", "Valley of Mexico", "Log persons per hectare"), custom.note = "SEs in parentheses", include.intercept = TRUE, reorder.coef = c(10, 2, 3, 4, 5, 6, 7, 8, 9, 1), groups = list("Plot size and crop yields" = 1:2, "Demographic variables" = 3:5, "Resource deprivation" = 6, "Family structure" = 7, "Access to medical attention" = 8, "Geographical region" = 9), include.dispersion = TRUE) 
          
extract(One.a, include.loglik = TRUE, include.nobs = TRUE, include.dispersion = TRUE, include.rsquared = TRUE)
```
...
```{r, warning=FALSE, error=FALSE}
Three <- glm.nb(hom.rate ~ sd.elev + pct.subs + pct.cattle + pct.ej + pct.com + log.corn.yield + log.pop.dens + pct.young + pct.indi + no.lit.rate + pct.fem.house + docs.per.10k + dummy.SOM, data = sample) #we are missing three independent variables in this model: % coffee production, % agricultural units 5 hectares or less, and Thiel's land inequality
