...
## add NAs to all missing values in the size vector in orde to identify, 
## municipal totals
plot_size$size[plot_size$size == ""] <- NA

## substitute with NA the "*" present in some observations for units, and 
## strip out all blank space from the units vector
...
plot_upto5 <- left_join(total_units, plot_upto5, by = "muncode") %>%
  select(muncode, plot.total, units.to5)
...
df.main <- df.main %>%
  mutate(prop.less.2500 = pop.less.2500/total.pop, pct.indi = (indi/total.pop) * 100, 
         ...
         log.pop.surface = log(total.pop/total_area), 
...
## calculate thiel index
## T = 1/u_tot * (summation from i=1 to G) (u_i)z_iln(z_i), 
## where z_i = (s_i/u_i)/(s_tot/u_tot) is the ratio of the 
## average plot size for agricultural units in plot size category 
##i (their entire surface area s_i over the number of units u_tot) 
## and G is the number of plot size categories

thiel_prep <- left_join(plot_size, total_units, by = "muncode") %>%
  filter(!is.na(size)) %>%
  select(muncode, thiel.area = area.ha, thiel.units = units, thiel.area.total = area.total, plot.total)

thiel_index <- filter(thiel_prep, !(muncode %in% 20001:20570))

thiel_index <- thiel_index %>%
  mutate(z1 = (thiel.area/thiel.units)/(thiel.area.total/plot.total)) %>% # calculate z1
  group_by(muncode, plot.total) %>%
  summarise(summation = sum((thiel.units*z1)*log(z1), na.rm = TRUE)) %>% #calculate summation
  mutate(thiel = (1/plot.total)*summation) %>%
  select(muncode, thiel)

## join thiel index with main data frame
df.main <- left_join(df.main, thiel_index, by = "muncode")

## separate thiel prep totals for Oaxaca
...
catalog <-read.dbf("data/cigel_1990_hab.dbf")
...
elev <- catalog %>%
  select(state = CVE_ENT, mun = CVE_MUN, elev = ALTITUD) %>%
  mutate(state = as.numeric(as.character(state)), mun = as.numeric(as.character(mun)), 
         elev = as.numeric(as.character(elev)), muncode = (state*1000) + mun) 
...
oaxaca.dist <- oaxaca.dist  %>% 
  ...
  summarise(pop.less.2500 = sum(pop.less.2500), total.pop = sum(total.pop), prop.less.2500 = pop.less.2500/total.pop,
            ...
            ejidal = sum(ejidal), total_area = sum(total_area), log.pop.surface = log(total.pop/total_area), 
            pct.ej = (ejidal/total_area) * 100,
            ...
            plot.total = sum(plot.total), units.to5 = sum(units.to5), pct.upto5 = (units.to5/plot.total) * 100) %>%
...
##calculate thiel index for oaxaca by distrito.

##join distritots with total units for oaxaca, calculate values for distritos
...
##join distrtidos and plot dized for oaxaca, calculate units and area by size for distritos
...
thiel_prep.oax <- left_join(plot_size.oax, total_units.oax, by = "distrito")
...
thiel_index.oax <- thiel_prep.oax %>%
  ...
  mutate(thiel = (1/plot.total)*summation) %>%
  select(distrito, thiel) %>%
  mutate(muncode = distrito + 20000)
  
#join theil index for oaxaca with oaxaca distrito data frame
oaxaca.dist <- left_join(oaxaca.dist, thiel_index.oax, by = "muncode")
...
sample.main <- sample.main %>%
  select(muncode, total.pop, dummy.SOM, prop.less.2500, pct.indi, no.lit.rate, docs.per.10k, pct.fem.house, pct.young, log.pop.dens, hom.rate, pct.ej, pct.com, pct.individual, pct.subs, log.corn.yield, pct.cattle, sd.elev, log.pop.surface, pct.upto5, pct.coffee, thiel)
...
#map.dbf <-read.dbf("data/maps/old_municipios1995-distritos.dbf")
#map.dbf <- map.dbf %>%
...
map_geom$elev_breaks <- cut(map_geom$sd_elev, breaks = breaks_elev, labels = labels_elev, include.lowest = TRUE)
...
```{r}
One.a <- glm.nb(hom.rate ~ log.corn.yield + log.pop.dens + pct.young + pct.indi + no.lit.rate + pct.fem.house + docs.per.10k + dummy.SOM, data = sample) #we are missing two of the three independent variables in this model: % agricultural units 5 hectares or less, and Thiel's land inequality

summary(One.a)

One.b <- glm.nb(hom.rate ~ log.corn.yield + log.pop.dens + pct.young + pct.indi + no.lit.rate + pct.fem.house + docs.per.10k + dummy.SOM, data = sample) #we are missing two of the three independent variables in this model: log average plot size, and Thiel's land inequality

summary(One.b)

One.c <- glm.nb(hom.rate ~ log.pop.surface + log.corn.yield + log.pop.dens + pct.young + pct.indi + no.lit.rate + pct.fem.house + docs.per.10k + dummy.SOM,data = sample) #we are missing one of the three independent variables in this model: Thiel's land inequality
 
summary(One.c)

htmlreg(list(One.a, One.b, One.c), caption = "Table 1: Coefficients from the Negative Binomial Regression Models of Homicide on Land Scarcity and Inequality (Hypotheses 1 and 2)", caption.above = TRUE, stars = c(.05, .01), custom.coef.names = c("Constant", "Log maize yields", "Log population density", "% young males", "% indigenous", "Illiteracy rate", "% female-headed households", "Medical doctors per 10,000", "Valley of Mexico", "Log persons per hectare"), custom.note = "SEs in parentheses,         * P < .05, ** P < .01, two-tailed tests", include.intercept = TRUE, reorder.coef = c(10, 2, 3, 4, 5, 6, 7, 8, 9, 1), groups = list("Plot size and crop yields" = 1:2, "Demographic variables" = 3:5, "Resource deprivation" = 6, "Family structure" = 7, "Access to medical attention" = 8, "Geographical region" = 9), include.aic = FALSE, include.bic = FALSE, include.deviance = FALSE, custom.gof.names = c("Log likelihood", "N")) 

# stargazer(One.a, One.b, One.c, type = "text", title = "Table 1: Coefficients from the Negative Binomial Regression Models of Homicide on # Land Scarcity and Inequality", style = "ajs", covariate.labels=c("Plot size and crop yields: Log persons per hectare", ". Log maize 
# yields", "Demographic variables: Log population density", ". % young males", ". % indigenous", "Resource Deprivation: Illiteracy rate", 
# "Family Structure: % female-headed households", "Access to medical attention: Medical doctors per 10,000", "Geographical Region: Valley 
# of Mexico"), dep.var.labels.include = FALSE, column.labels = c("Model 1", "Model 2", "Model 3"), model.numbers = FALSE, keep.stat = 
# c("n", "ll", "chi2"), notes = "SEs in parentheses")

#NOTE: I found the texreg functions to be better than stargazer in making Villarreal's tables. However, I haven't figured out how to include the overdispersion parameter alpha or chi-squared statistics. In order to view the tables before knitting, the function has to be changed to screenreg()
...
Two.a <- glm.nb(hom.rate ~ pct.ej + pct.com + log.corn.yield + log.pop.dens + pct.young + pct.indi + no.lit.rate + pct.fem.house + docs.per.10k + dummy.SOM, data = sample) #we are missing two independent variables in this model: % agricultural units 5 hectares or less, and Thiel's land inequality
 
summary(Two.a)
 
Two.b <- glm.nb(hom.rate ~ pct.individual + pct.cattle + pct.ej + pct.com + log.corn.yield + log.pop.dens + pct.young + pct.indi + no.lit.rate + pct.fem.house + docs.per.10k + dummy.SOM, data = sample) #we are missing three independent variables in this model: % coffee production, % agricultural units 5 hectares or less, and Thiel's land inequality
 
summary(Two.b)
 
Two.c <- glm.nb(hom.rate ~ pct.subs + pct.cattle + pct.ej + pct.com + log.corn.yield + log.pop.dens + pct.young + pct.indi + no.lit.rate + pct.fem.house + docs.per.10k + dummy.SOM, data = sample) #we are missing three independent variables in this model: % coffee production, % agricultural units 5 hectares or less, and Thiel's land inequality
 
summary(Two.c)

htmlreg(list(Two.a, Two.b, Two.c), caption = "Table 2: Coefficients from the Negative Binomial Regression Models of Homicide on Collective Ownership, Agricultural Organization and Production of Cash Crops (Hypotheses 3 and 4)", caption.above = TRUE, stars = c(.05, .01), custom.coef.names = c("Constant", "% ejido surface area", "% communal surface area", "Log maize yields", "Log population density", "% young males", "% indigenous", "Illiteracy rate", "% female-headed households", "Medical doctors per 10,000", "Valley of Mexico", "% individual production", "% cattle production", "% subsistence agriculture"), custom.note = "SEs in parentheses,         * P < .05, ** P < .01, two-tailed tests", include.intercept = TRUE, reorder.coef = c(2, 3, 12, 14, 13, 4, 5, 6, 7, 8, 9, 10, 11, 1), groups = list("Collective ownership" = 1:2, "Organization of agricultural production" = 3:5, "Plot size and crop yields" = 6, "Demographic variables" = 7:9, "Resource deprivation" = 10, "Family structure" = 11, "Access to medical attention" = 12, "Geographical region" = 13), include.aic = FALSE, include.bic = FALSE, include.deviance = FALSE, custom.gof.names = c("Log likelihood", "N")) 
...
```{r, warning=FALSE, error=FALSE}
Three <- glm.nb(hom.rate ~ sd.elev + pct.individual + pct.cattle + pct.ej + pct.com + log.corn.yield + log.pop.dens + pct.young + pct.indi + no.lit.rate + pct.fem.house + docs.per.10k + dummy.SOM, data = sample) #we are missing three independent variables in this model: % coffee production, % agricultural units 5 hectares or less, and Thiel's land inequality
 
summary(Three)

htmlreg(Three, caption = "Table 3: Coefficients from the Negative Binomial Regression Models of Homicide on Variation in Altitude (Hypothesis 5)", caption.above = TRUE, stars = c(.05, .01), custom.coef.names = c("Constant", "SD of altitude", "% individual production", "% cattle production", "% ejido surface area", "% communal surface area", "Log maize yields", "Log population density", "% young males", "% indigenous", "Illiteracy rate", "% female-headed households", "Medical doctors per 10,000", "Valley of Mexico"), custom.note = "SEs in parentheses,         * P < .05, ** P < .01, two-tailed tests", include.intercept = TRUE, reorder.coef = c(2, 5, 6, 3, 4, 7, 8, 9, 10, 11, 12, 13, 14, 1), groups = list("Local topography" = 1, "Collective ownership" = 2:3, "Organization of agricultural production" = 4:5, "Plot size and crop yields" = 6, "Demographic variables" = 7:9, "Resource deprivation" = 10, "Family structure" = 11, "Access to medical attention" = 12, "Geographical region" = 13), include.aic = FALSE, include.bic = FALSE, include.deviance = FALSE, custom.gof.names = c("Log likelihood", "N")) 
```
