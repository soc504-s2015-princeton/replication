...
oaxaca.hom <- left_join(distritos, df.oax, by = "muncode")  %>%  
    group_by(distrito)  %>% 
...
homs.map <- rbind(subset(homs.for.map, select = com.hom), subset(oaxaca.hom, select = com.hom))
...
##create maps, using 1995 data because difference between 1995 and 1990 data is negligible
library(ggplot2)
library(classInt)
gpclibPermit()
...
<<<<<<< HEAD
#write.dbf(map.dbf, "data/mgm1995/old-Municipios_1995.DBF") #NOT WORKING
#write.dbf(map.merged, "data/mgm1995/Municipios_1995.DBF") #NOT WORKING
=======
write.dbf(map.dbf, "data/mgm1995/Old_Municipios_1995.DBF") 
write.dbf(map.merged, "data/mgm1995/Municipios_1995.DBF")
>>>>>>> 385db2399baac075e53c1d742a8649806f8ad4a2
...
map_geom$hom_breaks <- cut(map_geom$hom_rate, breaks = breaks_hom, labels = labels_hom)

map_geom$elev_breaks <- cut(map_geom$sd_elev, breaks = breaks_elev, labels = labels_elev)
...
One.c <- glm.nb(hom.rate ~ log.pop.surface, data = sample) #we are missing one of the three independent variables in this model: Thiel's land inequality
+ log.corn.yield + log.pop.dens + pct.young + pct.indi + no.lit.rate + pct.fem.house + docs.per.10k + dummy.SOM,
summary(One.c)
#ADD STARGAZER
```
